...
public class Collision
{
	...
	public void CheckAllCollisions()
	{
		...
		for (Entity movEntity : movEntities)
		{
			CheckSingleCollision(movEntity);
		}
	}

	public void CheckSingleCollision(Entity movEntity)
	{
		...
		for (Entity entity : entities)
		{
			...
			if (true)
			//if (movableEl['isCircle'] && solidEl['isCircle']) // is both circles?
			{
	...
	private void CircleCollisionPush(Entity movEntity, Entity entity, float totalRadius, float solidOffsetX, float solidOffsetY, boolean reverseOffset)
	{
		...
		if (totalRadius > disPoints)
		{
			...
			movEntityPos.setX(movEntityPos.getX() + movablePosX < solidPosX ? -pushPos_b : pushPos_b);
		}
	}
}
